...
# alerts
  ...
  # else
    if @events.size == 1
      flash.now[:alert] = "Seule une pièce de théâtre correspond à votre recherche. Pour afficher profiter de notre sélection complète, modifiez vos critères !"
    # elsif @events.size == 0
    #   flash[:alert] = none
    elsif @events.size < 5
      flash.now[:alert] = "Seules #{@events.size} pièces de théâtre correspondent à votre recherche. Pour afficher profiter de notre sélection complète, modifiez vos critères !"
    end
